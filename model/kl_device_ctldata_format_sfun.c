/*
 * File: kl_device_ctldata_format_sfun.c
 *
 *
  *
...
#define S_FUNCTION_NAME kl_device_ctldata_format_sfun
/*<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<*/
...
#define NPARAMS              6
/* Parameter  1 */
#define PARAMETER_0_NAME      dev_type
...
extern void kl_device_ctldata_format_sfun_Outputs_wrapper(const real_T *u0,
                          const real_T *u1,
                          const real_T *u2,
                          const real_T *u3,
                          real_T *y0,
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
    ...
    const real_T   *u3  = (const real_T*) ssGetInputPortSignal(S,3);
    ...
    const real_T  *dev_type  = (const real_T *)mxGetData(PARAM_DEF0(S));
    const real_T  *p0  = (const real_T *)mxGetData(PARAM_DEF1(S));
    const real_T  *p1  = (const real_T *)mxGetData(PARAM_DEF2(S));
    const real_T  *p2  = (const real_T *)mxGetData(PARAM_DEF3(S));
    const real_T  *p3  = (const real_T *)mxGetData(PARAM_DEF4(S));
    const real_T  *p4  = (const real_T *)mxGetData(PARAM_DEF5(S));
    const int_T        y_width = ssGetOutputPortWidth(S,0);
    const int_T        u_width = ssGetInputPortWidth(S,0);

    kl_device_ctldata_format_sfun_Outputs_wrapper(u0, u1, u2, u3, y0, y1, y2, y3, dev_type, p_width0, p0, p_width1, p1, p_width2, p2, p_width3, p3, p_width4, p4, p_width5, y_width, u_width);
}
